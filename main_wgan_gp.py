...
def train():
    ...
    with tf.name_scope('INPUT_LR'):
        t_image = tf.placeholder('float32', [batch_size, 20, 20, 3], name='t_image_input_to_SRGAN_generator')
    ...
    for epoch in range(0, n_epoch+1):
        ...
        if (epoch != 0) and (epoch % 10 == 0):
            out = sess.run(net_g_test.outputs, {t_image: sample_imgs_96})#; print('gen sub-image:', out.shape, out.min(), out.max())
            summary = sess.run(merged,{t_image: sample_imgs_96,t_target_image:sample_imgs_384})
...
def evaluate():
    ...
    valid_hr_img_list = sorted(tl.files.load_file_list(path=config.VALID.lr_img_path, regx='.*.jpg', printable=False))
    valid_lr_img_list = sorted(tl.files.load_file_list(path=config.VALID.lr_img_path, regx='.*.jpg', printable=False))
    ...
    #valid_lr_imgs = scipy.misc.imresize(valid_lr_imgs_h, [size[0]/4, size[1]/4], interp='bicubic', mode=None)
    #valid_lr_imgs= tl.prepro.threading_data(valid_lr_imgs_h, fn=downsample_fn)

    # for im in valid_lr_imgs:
    #     print(im.shape)
    valid_hr_imgs = read_all_imgs(valid_hr_img_list, path=config.VALID.lr_img_path, n_threads=32)
    # for im in valid_hr_imgs:
    #     print(im.shape)
    # exit()
    ...
    valid_lr_img = imresize(valid_lr_img, size=[size[0]/8, size[1]/8], interp='bicubic', mode=None)
    valid_lr_img = (valid_lr_img / 127.5) - 1
    #valid_lr_img = scipy.misc.imresize(valid_lr_img, [size[0]/4, size[1]/4], interp='bicubic', mode=None)
    size = valid_lr_img.shape
